...
/*  	 d3.min(data, function(d) {
  ...
  yScale.domain(data.map(function(d) { return d.genre; } ));
  ...
  barchart.append("g")
      .attr("class", "y axis")
      .attr("transform", "translate(" + (margin.left + 65)  +  ",0)") // Moving the axis to fit in genre names
      .call(yAxis)
      .append("text")
	  ...
	  .text("Average failure rate (in %) over the last 5 years by genre");
  ...
  // Rendering the rectangles
    ...
    rects.attr("x", margin.left + 70) // Moving the x so genre name can fit
	    ...
	    .append("title")
		.text(function(d) {
			return d.genre + "'s failure rate over the last 5 years is " + d.avg + "%";
